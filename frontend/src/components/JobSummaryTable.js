...
const JobSummaryTable = ({ jobSummary }) => {
    ...
    return (
        <div className="flex flex-col">
            <table className="table-auto mt-4 border border-slate-500">
                <thead>
                <tr>
                    <th className="px-4 py-2 border border-slate-500 ">Job Title</th>
                    <th className="px-4 py-2 border border-slate-500 ">Average Salary</th>
                </tr>
                </thead>
                <tbody>
                {entries.slice(startIndex, endIndex).map(([jobTitle, averageSalary], index) => (
                    <tr key={jobTitle} className={(index % 2 === 0 ? 'bg-gray-100' : 'bg-white')}>
                        <td className="px-4 py-2 border border-slate-500 ">{jobTitle}</td>
                        <td className="px-4 py-2 border border-slate-500">{averageSalary} DH</td>
                    </tr>
                ))}
                </tbody>
            </table>
